...
export class Palette {
  ...
  constructor(private readonly state: () => GameState, private readonly hint: (message: string) => void) {
    ...
    this.offs.push(bus.on('budget:updated',()=>this.refreshItems()),bus.on('stats:updated',()=>this.refreshItems()),bus.on('milestone',()=>this.refreshItems()));
  }
...
function friendlyToolName(tool:ToolId):string {
  if(tool.startsWith('build_'))return BY_TOOL_BUILD_NAME(tool.slice(6));
  if(tool.startsWith('zone_'))return titleWords(tool.slice(5));
  return titleWords(tool);
}
function BY_TOOL_BUILD_NAME(key:string):string { return CATALOG.find(d=>d.key===key)?.name??titleWords(key); }
function titleWords(value:string):string { return value.split('_').filter(Boolean).map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(' ')||'Inspect'; }
