...
export class UI {
  ...
  constructor(private readonly root:HTMLElement,private readonly host:UIHost){
    ...
    this.panels=new Panels(host,this.toasts);root.append(this.panels.element);
    ...
    this.palette=new Palette(()=>host.state);this.hud.drawerMount().append(this.palette.element);
  ...
  promptSignText():Promise<string|null>{return new Promise(resolve=>{const back=document.createElement('div');back.className='modal-back';back.dataset.ui='';back.innerHTML='<form class="modal"><h3>Place a city sign</h3><input class="tin" maxlength="24" autocomplete="off" placeholder="Sign text"><div class="acts"><button type="button" class="btn cancel">Cancel</button><button class="btn primary">OK</button></div></form>';this.root.append(back);const input=back.querySelector('input')!;const done=(v:string|null)=>{back.remove();resolve(v);};back.querySelector('.cancel')!.addEventListener('click',()=>done(null));back.querySelector('form')!.addEventListener('submit',e=>{e.preventDefault();const v=input.value.trim();done(v||null);});requestAnimationFrame(()=>input.focus());});}
